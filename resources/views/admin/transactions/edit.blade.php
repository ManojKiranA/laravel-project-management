            ...
            <div class="form-group {{ $errors->has('project_id') ? 'has-error' : '' }}">
                ...
                <select name="project_id" id="project" class="form-control select2">
                    @foreach($projects as $id => $project)
                        <option value="{{ $id }}" {{ (isset($transaction) && $transaction->project ? $transaction->project->id : old('project_id')) == $id ? 'selected' : '' }}>
                            {{ $project }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('transaction_type_id') ? 'has-error' : '' }}">
                ...
                <select name="transaction_type_id" id="transaction_type" class="form-control select2">
                    @foreach($transaction_types as $id => $transaction_type)
                        <option value="{{ $id }}" {{ (isset($transaction) && $transaction->transaction_type ? $transaction->transaction_type->id : old('transaction_type_id')) == $id ? 'selected' : '' }}>
                            {{ $transaction_type }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('income_source_id') ? 'has-error' : '' }}">
                ...
                <select name="income_source_id" id="income_source" class="form-control select2">
                    @foreach($income_sources as $id => $income_source)
                        <option value="{{ $id }}" {{ (isset($transaction) && $transaction->income_source ? $transaction->income_source->id : old('income_source_id')) == $id ? 'selected' : '' }}>
                            {{ $income_source }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('currency_id') ? 'has-error' : '' }}">
                ...
                <select name="currency_id" id="currency" class="form-control select2">
                    @foreach($currencies as $id => $currency)
                        <option value="{{ $id }}" {{ (isset($transaction) && $transaction->currency ? $transaction->currency->id : old('currency_id')) == $id ? 'selected' : '' }}>
                            {{ $currency }}
                        </option>
                    @endforeach
                </select>
